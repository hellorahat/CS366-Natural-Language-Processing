...
class BigramModel:
    ...
    def train(self, corpus):
        logging.info("Starting BigramModel.train({})".format(corpus.name))
        # Reset file pointer
        corpus.seek(0)
        for sentence in corpus:
            for word in sentence.split():
                if word != "<s>":  # Don't include <s>
                    ...
                    self.total_words += 1
        logging.info("Finished BigramModel.train({})".format(corpus.name))
    ...
    def calculate_probability(self, previous_word, current_word):
        ...
        if previous_word_frequency == 0:
            return 0.0
        return current_word_frequency/previous_word_frequency
